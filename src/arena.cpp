...
void Arena::generateRandomNpcs(int count) {
    ...
    const std::vector<std::string> types = {"Elf", "Rogue", "KnightErrant"};
    ...
    for (const auto& [name, npc] : npcs_) {
        if (npc->isAlive()) {
            ...
            if (x >= 0 && x <= width_ && y >= 0 && y <= height_) {
                char symbol = '?';
                if (npc->getType() == "Elf") symbol = 'E';
                else if (npc->getType() == "Rogue") symbol = 'R';
                else if (npc->getType() == "KnightErrant") symbol = 'K';
                map[y][x] = symbol;
            }
        }
    }
